...
const Checkbox = ({value, id, chosen}) => {
  ...
  return(
    <input className="form-check-input" type="checkbox" value={value} id={id} onChange={handleChecked} defaultChecked={false} checked={checked}/>
  )
}
...
const Filters = ({ products, setSearchResults }) => {
    ...
    return (
      <div  className="collapse text-center bg-light fixed-top min-vh-100" id="filters">
        <nav className="navbar navbar-dark navbar-expand-sm text-white bg-dark">
          <div className="container-fluid d-flex ">
            <button data-bs-toggle="collapse" data-bs-target="#filters" className="btn btn-dark"><FontAwesomeIcon icon={faChevronLeft} /></button>
            <h3>Фильтры:</h3>
            <div></div>
          </div>
        </nav>
            ...
            <div id="flush-collapseOne" className="accordion-collapse collapse show" aria-labelledby="flush-headingOne" data-bs-parent="#accordionFlushExample">
              <div className="accordion-body">
                <div className="priceFilter d-flex justify-content-between align-items-center">
                  от <input type="number" className="form-control rounded-pill ms-1" id="lowPrice" value={lowPrice} onChange={(e) => setLowPrice(parseInt(e.target.value))}/>
                  
                  <svg height={2} width={100} className="mx-1">
                    <line x1={0} y1={0} x2={100} y2={0} style={{stroke: 'rgb(0,0,0)', strokeWidth: 2}} />
                  </svg>
                  до <input type="number" className="form-control rounded-pill ms-1" id="highPrice" value={highPrice} onChange={(e) => setHighPrice(parseInt(e.target.value))}/>  
                </div>
              </div>
            </div>
          </div>
          <div className="accordion-item">
            ...
            <div id="flush-collapseTwo" className="accordion-collapse collapse" aria-labelledby="flush-headingTwo" data-bs-parent="#accordionFlushExample">
              <div className="accordion-body">
                ...
                {/* Sizes show */}
                {sizes?.map((size, i) => (
                  <div className="size me-3 d-flex align-items-center" key={i}>
        ...
        <button className="btn btn-danger my-2 mx-2 rounded-pill" data-bs-toggle="collapse" data-bs-target="#filters" onClick={clearFilter}>Сбросить Фильтры</button>
        
        <input type="submit" value="Применить" data-bs-toggle="collapse" data-bs-target="#filters" className="btn btn-primary rounded-0"/>
        </form>
      </div>
    )
}
